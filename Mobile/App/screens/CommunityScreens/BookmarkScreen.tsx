...
const BookmarkScreen = ({ route, navigation }: any) => {
    ...
    const Addbookmark = async (user_pk: any, post_pk: any) => {
        try {
            const response = await fetch('http://192.168.35.83:3000/add_book_mark', {
                method: 'POST',
                headers: {
    ...
    const getGeneralposts = async () => {
        try {
            const response = await fetch('http://192.168.35.83:3000/generalpost');
    ...
    const getDepartmentposts = async () => {
        try {
            const response = await fetch('http://192.168.35.83:3000/departmentpost');
            const postsdata = await response.json();
            setCommunityData(postsdata);
        } catch (error) {
            //console.error(error)
    ...
    const AreYouHavePost = async () => {
        try {
            const response = await fetch('http://192.168.35.83:3000/get_user_have_post', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
